  ...
  return (
    <div className="receipt-container hidden print:block" style={{ width: '300px', fontFamily: 'monospace', fontSize: '12px', lineHeight: '1.3', color: 'black', backgroundColor: 'white', padding: '10px', margin: '0 auto' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '6px', fontSize: '11px' }}>
        ...
        <tbody>{sale.items.map((item, idx) => (<tr key={idx}><td style={{ verticalAlign: 'top', paddingTop: '4px', whiteSpace: 'nowrap' }}>{item.cantidad.toFixed(3)}</td><td style={{ verticalAlign: 'top', paddingTop: '4px', paddingRight: '4px' }}>{item.descripcion}{item.descuento_porcentaje > 0 && <span style={{ fontSize: '9px', color: '#c70' }}> -{item.descuento_porcentaje}%</span>}</td><td style={{ verticalAlign: 'top', paddingTop: '4px', textAlign: 'right', whiteSpace: 'nowrap' }}>${fmt2(item.subtotal)}</td></tr>))}</tbody>
      </table>
      <div style={{ borderBottom: '1px dashed #666', margin: '6px 0' }} />
